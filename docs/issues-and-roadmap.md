...
- **问题**: API Key `sk-vaqj...vhoe` 被直接写入 YAML 并提交到 Git 仓库
- **影响**: 任何有仓库访问权限的人都能获取该 Key；若仓库公开则风险更大
- **建议**:
  1. 立即轮换该 API Key
  2. 将 `providers.yaml` 加入 `.gitignore`
  3. 提供 `providers.yaml.sample` 作为模板（不含真实 Key）
  4. 改为从环境变量或 `.env` 文件读取 API Key

#### 2. Provider API 无认证保护

- **位置**: `src/webgal_agent/api/routes/provider.py`
- **问题**: PUT `/api/providers/{agent_name}` 和 PUT `/api/providers` 可修改 LLM 配置（含 API Key），无任何鉴权
- **影响**: 任何能访问 API 的人都可修改提供商配置，甚至注入恶意 `base_url`
- **建议**: 至少添加 API Key / Bearer Token 认证中间件

#### 3. Provider API 返回 API Key 明文

- **位置**: `src/webgal_agent/api/routes/provider.py` 第 23 行
- **问题**: `cfg.model_dump()` 直接返回含 `api_key` 的完整字典
- **影响**: 即使前端做了遮挡，HTTP 响应体仍包含明文 Key
- **建议**: 返回时对 `api_key` 字段做脱敏处理（仅返回后 4 位）

#### 4. TaskManager 不支持并发任务

- **位置**: `src/webgal_agent/api/task_manager.py`
- **问题**: `_active_agents` 和 `_running_task_id` 是单例，只记录一个运行中任务。如果用户同时启动两个任务的 `run_step`，后者会覆盖前者，导致 cancel 逻辑失效
- **建议**: 改为 `dict[str, dict[str, Agent]]` 结构，支持多任务并发运行
...
#### 9. `_call_llm_with_tools` 最大轮次边界问题
...
#### 13. `update_step_result` 只更新第一条匹配消息
...
#### 15. `KnowledgeEntry.category` 类型不一致
...
#### 18. `_load_prompts` 和 `_load_knowledge_requirements` 重复读取同一文件

- **位置**: `src/webgal_agent/api/task_manager.py` 第 144-187 行
- **建议**: 合并为一次读取，缓存结果

#### 19. 每次 `run_step` 都重建所有 Agent

- **位置**: `src/webgal_agent/api/task_manager.py` 第 527 行
- **建议**: 缓存或复用 Agent 实例

#### 20. `get_workflow_info` 每次调用都构建 Agent
